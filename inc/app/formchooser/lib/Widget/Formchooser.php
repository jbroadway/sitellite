...
// field that uses the formchooser app.
//
...
/**
	 ...
	 * <?php
	 * 
	 * $widget = new MF_Widget_formchooser ('name');
	 ...
	 * @version	1.0, 2003-07-31, $Id: Formchooser.php,v 1.1 2008/02/20 10:28:55 lux Exp $
	 * @access	public
	 * 
	 */

class MF_Widget_formchooser extends MF_Widget {
	/**
	 ...
	 * 
	 */
	function MF_Widget_formchooser ($name) {
		parent::MF_Widget ($name);
		$this->passover_isset = true;
	}

	function _link () {
		// display a button that pops up the formchooser app
		static $included = false;
		if (! $included) {
			page_add_script (site_prefix () . '/js/dialog.js');
			page_add_script (loader_box ('formchooser/js'));
			$included = true;
		}
		return template_simple ('
			<script language="javascript" type="text/javascript">
				...
				function formchooser_handler () {
					...
					formchooser_form.elements[formchooser_element].value = url;
				}

			</script>

			<input type="submit" onclick="formchooser_get_file (this.form, \'{name}\'); return false" value="{intl Choose}" />
		', $this);
	}
