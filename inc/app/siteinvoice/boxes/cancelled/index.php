...
foreach ($cgi->_key as $key) {
	db_execute ('update siteinvoice_invoice set status = "cancelled" where id = ?', $key);
...
*/
	...
	$mailer->From = appconf ('company_email');
	$mailer->FromName = appconf ('company_email_name');
	$mailer->Subject = 'Cancelled Invoice #' . $key;
	$mailer->Body = template_simple ('email/cancelled.spt', $client);
...
page_title ('SiteInvoice - Invoices Cancelled');
