...
class SitelinksAddForm extends MailForm {
	function SitelinksAddForm () {
		parent::MailForm ();

		$this->parseSettings ('inc/app/sitelinks/forms/add/settings.php');
		
		global $page, $cgi;
		
		page_title (intl_get ('Adding Link'));
			...
			function cms_cancel (f) {
				...
				if (arguments.length == 0) {
					...
					if (f.elements["_return"] && f.elements["_return"].value.length > 0) {
						...
						window.location.href = "/index/sitelinks-app";
					}
				}
				return false;
			}
	...
	function onSubmit ($vals) {
		...
		header ('Location: ' . site_prefix () . '/index/sitelinks-app/id.' . $res);
		exit;
	}
}
