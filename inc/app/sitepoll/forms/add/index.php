...
class SitepollAddForm extends MailForm {
	function SitepollAddForm () {
		parent::MailForm ();

		loader_import ('saf.GUI.Menu');
		
		$this->parseSettings ('inc/app/sitepoll/forms/add/settings.php');
		
		global $page, $cgi;
		
		page_title (intl_get ('Adding Poll'));
			...
			function cms_cancel (f) {
				...
				if (arguments.length == 0) {
					...
					if (f.elements["_return"] && f.elements["_return"].value.length > 0) {
						...
						window.location.href = "/index/sitepoll-app";
					}
				}
				return false;
			}
	...
	function onSubmit ($vals) {
		...
		$vals['resultdate'] = preg_replace('/ \d\d:\d\d:\d\d/','',array($vals['resultdate']));
		$vals['resultdate'] = $vals['resultdate'][0];
		...
		if (! $res) {
			if (empty ($return)) {
				$return = site_prefix () . '/index/cms-browse-action?collection=sitepoll_poll';
			}
		...
		header ('Location: ' . site_prefix () . '/index/sitepoll-app/id.' . $res);
		exit;
	}
}
