...
if (! empty ($parameters['query'])) {
	...
	//echo '<p style="background-color: #fd0; padding: 5px; border: 1px solid #000; font-family: monospace">' . htmlentities ($res['metadata']['syntax']) . '</p>';
...
foreach (array_keys ($results) as $key) {
	...
	if (strstr ($results[$key]['url'], '?')) {
		$results[$key]['link'] = $results[$key]['url'] . '&highlight=' . $enc;
	} else {
		$results[$key]['link'] = $results[$key]['url'] . '?highlight=' . $enc;
	}
}
