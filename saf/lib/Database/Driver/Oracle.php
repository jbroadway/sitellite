...
/**
	 * Oracle_Driver is the Database driver for the Oracle database system.
	 * Oracle_Driver provides all of the Oracle-specific functionality
	 * for Query objects.  It is accessed through the Database class API.
	 * Note that you will need to compile PHP with the --with-oci8 option.
	 * 
	 * <code>
	 * <?php
	 * 
	 * $q = new Oracle_Driver ("select * from table");
	 * if ($q->execute ()) {
	 * 	while ($row = $q->fetch ()) {
...
class Oracle_Driver extends Query {
	/**
	 ...
	 * 
	 */
	function Oracle_Driver ($sql = '', &$conn, $cache = 0) {
	...
	function execute () {
		...
		// query the database
		$this->result = ociparse ($this->connection, $this->tmp_sql);
		if (@ociexecute ($this->result, $this->mode)) {
			return $this->result;
		}
		return false;
	}
	...
	function field ($num = 0) {
		if ($this->result) {
			if (! is_array ($this->fields)) {
				return @ocicolumnname ($this->result, $num);
			} else {
	...
	function _fetch () {
		...
		if (db_fetch_mode () == DB_FETCHMODE_OBJECT) {
			return (object) $row;
		}
		return $row;
	}
	...
	/**
	 * Issues a "begin" statement, beginning a transaction.
	 * 
	 * @access	public
	 * 
	 */
	...
	/**
	 * Issues a "commit" statement, committing the current transaction.
	 * 
	 * @access	public
	 * 
	 */
	...
	/**
	 * Issues a "rollback" statement, rolling back the current transaction.
	 * 
	 * @access	public
	 * 
	 */
...
/**
	 ...
	 * <?php
	 * 
	 * $db = new Oracle_Driver ("name", "host", "user", "pass", 1);
	 * 
	 * if ($db->connection) {
	 * 	// connection worked
	 * } else {
...
class Oracle_Driver {
	 ...
	 * 
	 */
	function Oracle_Driver ($name = "", $host = "", $user = "", $pass = "", $persistent = 1) {
		$this->name = $name;
		$this->host = $host;
		$this->user = $user;
		$this->pass = $pass;
	...
	function connect () {
		if ($this->persistent) {
			$this->connection = @ociplogon ($this->user, $this->pass, $this->name);
		} else {
			$this->connection = @ocilogon ($this->user, $this->pass, $this->name);
		}
	...
	function getTables (&$db) {
		$tables = array ();
		$res = ociparse ($this->connection, 'desc all_tables');
		if ($res) {
			if (ociexecute ($res)) {
				while (ocifetchinto ($res, $row, OCI_ASSOC | OCI_RETURN_NULLS)) {
	...
	/* Below not compatible with Oracle driver. */
...
/**
	 * Oracle_DatabaseTable provides all of the Oracle-specific functionality
	 * for the saf.Database.Table package.
	 * 
	 * <code>
	 * <?php
	 * 
	 * $dt = new Oracle_DatabaseTable ($db, 'tablename', 'pkeycolumn');
	 * 
	 * if ($res = $dt->fetchAll ()) {
...
class Oracle_DatabaseTable extends DatabaseTable {
	/**
