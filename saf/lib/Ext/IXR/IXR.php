    ...
    function call($methodname, $args) {
        ...
        // objects must first be registered via registerObject ('name', $obj)
          ...
          //  $method = substr($method, 5);
            ...
            //if ($result === false) {
            //	return new IXR_Error (-1, 'server error. ' . $this->registeredObjects[$object]->error);
            //}
        // </lux>
...
class IXR_Client {
    ...
    function query() {
		...
		if ($this->username !== false) {
			$request .= 'Authorization: Basic ' . base64_encode ($this->username . ':' . $this->password) . $r;
		}
        ...
        foreach ($types as $type) {
            switch ($type) {
                ...
                case 'double':
                    ...
                    break;
                case 'dateTime.iso8601':
                    $return[] = 'dateTime.iso8601'; //new IXR_Date(time());
                    break;
                case 'boolean':
                    $return[] = 'boolean';
                    break;
                case 'base64':
                    $return[] = 'base64'; //new IXR_Base64('base64');
                    break;
                case 'array':
                    $return[] = 'array'; //array('array');
                    break;
                case 'struct':
                    $return[] = 'struct'; //array('struct' => 'struct');
                    break;
            }
        }
        return $return;
