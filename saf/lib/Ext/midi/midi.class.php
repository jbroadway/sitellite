...
//---------------------------------------------------------------
// returns MIDI XML representation (v0.8, http://www.musicxml.org/dtds/midixml.dtd)
//---------------------------------------------------------------
...
<!DOCTYPE MIDIFile SYSTEM \"http://dasdeck.de/staff/valentin/midi/dtds/midixml.dtd\">
<!-- identical to http://www.musicxml.org/dtds/midixml.dtd, except for 2 bug fixes, 
which will be fixed by recordare in next version, v0.9 -->
<MIDIFile>
<Format>$type</Format>
  	...
  	for ($j=0;$j<$mc;$j++){
  		...
  		$xml .= '    ';
			...
			switch($msg[1]){
				...
				case 'SMPTE':
					$xml .= "<SMPTEOffset TimeCodeType=\"1\" Hour=\"{$msg[2]}\" Minute=\"{$msg[3]}\" Second=\"{$msg[4]}\" Frame=\"{$msg[5]}\" FractionalFrame=\"{$msg[6]}\"></SMPTEOffset>\n"; //TimeCodeType???
					break;
				...
				case 'SeqSpec':
					...
					$start = strpos($line,'"')+1;
					$end = strrpos($line,'"');
					$data = substr($line,$start,$end-$start);
