			...
			{if $info[p].subpackage == ""}
				...
				{if $info[p].classes}
					...
					{section name=class loop=$info[p].classes}
						<dd><img class="tree-icon" src="{$subdir}media/images/{if $info[p].classes[class].access == 'private'}Private{/if}Class.png" alt="Class"><a href='{$info[p].classes[class].link}' target='right'>{$info[p].classes[class].title}</a></dd>
					{/section}
				{/if}
				{if $info[p].functions}
