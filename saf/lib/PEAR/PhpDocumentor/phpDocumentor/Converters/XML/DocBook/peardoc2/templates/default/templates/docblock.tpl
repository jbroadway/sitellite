     ...
     <variablelist>
      <varlistentry>
       <term>
        ...
        &type.{$paramtype};
        {/if}
         <parameter>{$params[params].name}</parameter>
       </term>
       <listitem>
        <para>
...
{if $tagname != 'static' && $tagname != 'author' && $tagname != 'version' && $tagname != 'copyright' && $tagname != 'package' && $tagname != 'subpackage'}
   <{$num} id="{$id}.{$tagname}">
    &title.{$tagname};
...
{elseif $tags[tags].keyword == 'static'}
