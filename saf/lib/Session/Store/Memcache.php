...
// +----------------------------------------------------------------------+
//
// This is the session store driver for storing session variables in
// memcached.  This enables session variables to be shared across multiple
...
class SessionStore_Memcache extends SessionStore {
	 ...
	 * 
	 */
	function SessionStore_Memcache () {
...
function session_store_memcache_open ($save_path, $session_name) {
	if (! class_exists ('Memcache')) {
		return false;
	}
	$ini = parse_ini_file ('inc/sessions/sitellite/settings.php', true);
	$GLOBALS['session_store_memcache'] = new Memcache;
	return $GLOBALS['session_store_memcache']->connect (
		$ini['Store']['server'],
		$ini['Store']['port']
	);
}

function session_store_memcache_close () {
	return true;
}

function session_store_memcache_read ($key) {
	return $GLOBALS['session_store_memcache']->get ('session_store:' . $key);
}

function session_store_memcache_write ($key, $value) {
	return $GLOBALS['session_store_memcache']->set ('session_store:' . $key, $value);
}

function session_store_memcache_destroy ($id) {
	return $GLOBALS['session_store_memcache']->delete ('session_store:' . $id);
}

function session_store_memcache_gc ($maxlifetime) {
	return true;
}

session_set_save_handler (
	'session_store_memcache_open',
	'session_store_memcache_close',
	'session_store_memcache_read',
	'session_store_memcache_write',
	'session_store_memcache_destroy',
	'session_store_memcache_gc'
);
